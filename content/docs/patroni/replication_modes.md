...
Patroni 将根据 `synchronous_node_count` 参数，仅将一个或多个同步备库节点分配给 `synchronous_standby_names`。

在每次 HA 循环迭代中，Patroni 都会重新评估同步备库节点的选择。如果当前同步备库节点列表处于连接状态，且没有请求解除其同步状态，则继续保留。否则，将选择可用于同步且复制进度最靠前的集群成员。
