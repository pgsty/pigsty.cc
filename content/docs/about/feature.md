...
- [**可扩展性**](/img/pigsty/ecosystem.png)：[**海量扩展**](/ext/list) 开箱即用：深度整合 **PostGIS**、**TimescaleDB**、**Citus**、**PGVector** 等 [**{{< param pgext_count >}}**](/ext/list/) 插件与 [**12 款 PG 内核**](/docs/pgsql/kernel)。
- [**可靠性**](/img/pigsty/arch.png)：快速创建 [**高可用**](/docs/concept/ha/)、故障自愈的 [**PostgreSQL**](/docs/pgsql) 集群，自动预置 [**时间点恢复**](/docs/concept/pitr/)、[**访问控制**](/docs/concept/sec/ac)、自签名 [**CA**](/docs/concept/sec/ca) 与 [**TLS**](/docs/concept/sec/ca)。
...
- [**可维护性**](/img/pigsty/iac.jpg)：[**简单易用**](/docs/setup/install)，[**基础设施即代码**](/docs/pgsql/config)，[**管理SOP预案**](/docs/pgsql/admin/)，自动调参，本地软件仓库，[**Vagrant**](/docs/deploy/vagrant) [**沙箱**](/docs/deploy/sandbox) 与 [**OpenTofu**](/docs/deploy/terraform) 模板，不停机 [**迁移**](/docs/pgsql/migration) 方案。
...
PostgreSQL 的灵魂在于其丰富的 [**扩展生态**](/blog/pg/pg-eat-db-world)，而 Pigsty 独一无二地深度整合了 PostgreSQL 生态中的 [**{{< param pgext_count >}} 扩展**](/ext/list/)，为您提供开箱即用的超融合多模态数据库！

插件间可以产生 [**协同效应**](/blog/pg/pg-eat-db-world#极致可扩展性的魔法)，产生 1+1 远大于 2 的效果。
...
[**安全与合规**](/docs/concept/sec/) 章节说明各项机制的默认状态和适用边界；[**安全考量**](/docs/deploy/security/) 提供生产加固建议；[**合规实践**](/docs/concept/sec/compliance) 给出等保与 SOC 2 的参考映射。是否满足具体合规要求取决于部署范围、组织流程、持续证据和审计结论。
