...
角色固定派生 `${kafka_data}/data` 与 `${kafka_data}/metadata`。该路径必须是专用绝对路径，不能是 `/`、`/data`、`/var`、`/etc`、`/opt`、`/usr`、`/home`、`/root` 或 `/pg`。[`kafka-rm.yml`](/docs/pilot/kafka/playbook#集群下线) 默认会删除整个根目录，因此不要混放其他服务或业务文件。
...
KRaft Controller 监听端口，默认 `9093`（Kafka KRaft 惯例端口），只在 Controller-capable 节点监听。注意它与 Pigsty Infra 节点上 Alertmanager 的默认端口相同：若 Kafka 与 Infra 复用节点，请为该集群调整此参数，角色不会自动检测跨服务端口冲突。
...
两种轮换动作互斥，且必须以精确完整集群为目标。`kafka-rm.yml` 默认删除数据目录与管理节点上的 Manifest/Secret/PKI 缓存，执行前必须显式确认目标集群与备份/重建意图。命令与完整语义见 [预置剧本](/docs/pilot/kafka/playbook)。
