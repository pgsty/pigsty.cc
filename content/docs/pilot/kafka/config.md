...
KAFKA 模块使用 16 项持久公开参数表达集群意图，其余拓扑、监听器、存储子目录、复制安全、授权与 Exporter 放置由角色统一推导。首次部署建议先完成 [快速上手](/docs/pilot/kafka/start)；完整字段见 [参数参考](/docs/pilot/kafka/param)。
...
- `9092`、`9095`、`9308`、`9404` 互不冲突，Infra 节点可以访问两个指标端口
...
- 每个节点显式使用 initial-controller 或 non-initial-controller 格式化模式；
- 启动后必须确认 `kraft.version` 已进入 dynamic 模式。

Bootstrap-only 事实保存在管理节点：

```text
files/kafka/<kafka_cluster>/manifest.yml
```

Manifest 只记录集群身份、初始 Controller Identity、安全模式和初始 RF/minISR。活集群始终是运行事实权威：

- Manifest 与现场身份或安全模式冲突时，普通剧本失败关闭；
- 旧 Manifest 存在但全部数据盘为空时拒绝复活旧集群；
- 健康 dynamic 集群缺少 Manifest 时，可以从现场重建而不重新格式化；
- `scram` 集群重建 Manifest 时还要求已有的角色自有安全材料完整。
...
已格式化节点会从 `${kafka_data}/metadata/meta.properties` 读取 `cluster.id`、`node.id` 与 Directory ID，并与 Manifest、清单及活 quorum 交叉校验。身份不匹配是保护性失败，不应通过删除 `meta.properties` 或清空数据绕过。
...
| [`kafka_controller_port`](/docs/pilot/kafka/param#kafka_controller_port) | `9095` | KRaft Controller 仲裁 |
...
| 所有 Kafka 成员 | 所有 Controller | `9095` | KRaft 元数据仲裁 |
| Infra/VictoriaMetrics | 所有启用 JMX 的节点 | `9404` | JVM/Kafka 指标 |
...
| `files/kafka/<cluster>/manifest.yml` | 管理节点上的 Bootstrap Manifest |
{.full-width}
