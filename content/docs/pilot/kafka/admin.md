...
```bash
ss -lntp | grep -E ':9092|:9095|:9308|:9404'
...
- 健康集群存在静态变化：严格 `serial: 1`，每节点重启前后执行 Controller 零 Lag/最近追平、quorum、Offline Partition、Under Min ISR 与 ISR 追平门禁；
- 没有静态变化：不重启 Kafka。

不要用 `-t kafka_config` 绕过完整状态机。动态 Topic/User/ACL/Quota 收敛位于 Finalize 阶段，静态变化是否重启由角色决定。
...
已格式化且健康的 `scram` 集群支持两种互斥的受保护动作：内部凭据轮换和证书轮换。两者都要求精确完整集群、匹配确认字符串和先执行 `--check`；证书轮换还会先验证节点时钟偏差与完整证书集合。
...
4. 在明确恢复/替换方案前，不执行 `kafka_clean`，不删除 `meta.properties`；
