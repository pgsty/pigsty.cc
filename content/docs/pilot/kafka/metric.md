...
KAFKA 模块使用两类指标源：`job=kafka` 采集每个 JVM 的内部状态，`job=kafka_exporter` 通过 Kafka 协议采集逻辑集群、Topic、Partition 与 Consumer Group 状态。协议 Exporter 只放在 `kafka_seq` 最小的至多两个 Broker-capable 节点上，单 Broker 集群只运行一个。
...
| 指标源 | 公共标签 |
|:---|:---|
| `job=kafka` | `job`, `cls`, `ins`, `ip`, `instance`, `role`, `node_id` |
| `job=kafka_exporter` | `job`, `cls`, `ins`, `ip`, `instance` |
{.full-width}
...
| 指标 | 类型 | 含义 |
|:---|:---:|:---|
| `kafka_up` | Gauge/Recording | `job=kafka` 的抓取可用性；带 Kafka 角色标签 |
| `kafka_exporter_up` | Gauge/Recording | `job=kafka_exporter` 的抓取可用性 |
...
以下指标来自 `job=kafka_exporter`。同一集群的多个 Exporter 会看到相同的逻辑集群状态，直接做集群聚合时必须按语义去重，不能简单把所有 `ins` 相加。
...
Dashboard 与告警使用方式参阅 [监控告警](/docs/pilot/kafka/monitor)。
