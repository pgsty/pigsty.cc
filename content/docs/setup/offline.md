...
下表记录 `v4.4.0` 历史离线制品及其制作时使用的操作系统小版本；这些版本不代表 [**当前推荐操作系统**](/docs/ref/linux/)。
...
社区版制品可以从 [**GitHub 发布页面**](https://github.com/pgsty/pigsty/releases/tag/v4.4.0) 下载。`v4.4.0` 全部离线包的 MD5 校验和如下：
...
> [!IMPORTANT]
> 当前 v4.5 开发源码的 `repo_create` 与 `cache_create` 都要求 SOW，并会执行 `sow create --pigsty`；当前候选软件包版本为 0.3.0。v4.4.0 正式离线包是旧发布制品，不能据此推断其中已经包含 SOW；用当前源码重建旧仓库前，应先从 Pigsty INFRA 仓库补齐匹配版本的 SOW，或刷新为匹配开发版本的离线介质。
...
| 参数               | 默认值                                       | 说明                     |
|:-----------------|:------------------------------------------|:-----------------------|
| `cache_pkg_name` | `pigsty-pkg-${version}.${os}.${arch}.tgz` | 离线包文件名模板               |
| `cache_pkg_dir`  | `dist/${version}`                         | 管理节点上的输出目录             |
| `cache_repo`     | `pigsty`                                  | 从目标节点打包的本地仓库；多个仓库用逗号分隔 |
