...
Pigsty 版本号字符串，默认值为当前版本：`v4.3.0`。

Pigsty 内部会使用版本号进行功能控制与内容渲染，请勿随意修改此参数。

Pigsty 使用语义化版本号，版本号字符串通常以字符 `v` 开头，例如 `v4.3.0`。
...
解析记录会记录在 Infra 节点的 `/etc/dnsmasq.d/pigsty/default` 文件中。 要使用这个 DNS 服务器，您必须将 `nameserver <ip>` 添加到 `/etc/resolv.conf` 中，[`node_dns_servers`](/docs/node/param#node_dns_servers) 参数可以解决这个问题。
