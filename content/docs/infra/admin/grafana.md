---
title: Grafana 高可用部署：使用 PostgreSQL 后端数据库
linkTitle: Grafana 高可用
weight: 4000
description: 使用 PostgreSQL 而不是 SQLite 作为 Grafana 后端使用的远程存储数据库，获取更好的性能与可用性。
...
实际上调用了 Ansible Playbook [`pgsql-user.yml`](/docs/pgsql/playbook#pgsql-useryml) 创建用户
...
实际上调用了 Ansible Playbook [`pgsql-db.yml`](/docs/pgsql/playbook#pgsql-dbyml) 创建数据库
...
您可以直接通过修改 Pigsty 配置文件，更改 Grafana 使用的后端数据源，一步到位的完成切换 Grafana 后端数据库的工作。编辑 `pigsty.yml` 中 [`grafana_pgurl`](/docs/infra/param#grafana_pgurl) 参数，将其修改为：

```yaml
grafana_pgurl: postgres://dbuser_grafana:DBUser.Grafana@meta:5436/grafana
```

然后重新执行 [`infra.yml`](/docs/infra/playbook#infrayml) 中的 `grafana` 任务，即可完成 Grafana 升级
