...
托管集群的编排式时间点恢复请优先使用 [`pig pitr`](/docs/pig/pitr/)。
...
**重要提示：** 恢复前必须先停止 PostgreSQL；如果该 PGDATA 由 Patroni 管理，应使用 [`pig pitr`](/docs/pig/pitr/) 编排 Patroni、PostgreSQL 与 pgBackRest：
