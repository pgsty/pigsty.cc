...
节点的数据目录由参数 [`node_data`](/docs/node/param#node_data) 指定，默认为 `/data`，由 `root` 用户持有，权限为 `0777`。
...
Prometheus 的主配置文件则位于 [`roles/infra/templates/prometheus/prometheus.yml.j2`](https://github.com/pgsty/pigsty/blob/main/roles/infra/templates/prometheus/prometheus.yml.j2) ，并渲染至所有基础设施节点的 `/etc/prometheus/prometheus.yml`。

VictoriaMetrics 相关的脚本与规则定义放置于 pigsty 主目录下的 [`files/victoria/`](https://github.com/pgsty/pigsty/tree/master/files/victoria) 目录，会被拷贝至所有基础设施节点的 `/etc/prometheus/` 下。
...
* [pg_fs_main](/docs/pgsql/param#pg_fs_main)：Postgres主数据盘挂载点，默认为`/data`
