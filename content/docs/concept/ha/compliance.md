...
本文聚焦业务连续性与灾难恢复；访问控制、通信加密、审计等安全控制的通用映射见 [**合规实践**](/docs/concept/sec/compliance)。
...
| 配置                           | RTO             | RPO              | 对应国标等级    |
|:-----------------------------|:----------------|:-----------------|:----------|
| 默认异步（`pg_rto: norm`）         | 目标 < 45s        | 可能丢失；候选采样阈值 1MiB | 需结合完整架构评估 |
| 最大可用同步（`norm` + `pg_rpo: 0`） | 目标 < 45s        | 正常 0；无同步副本时可降级   | 需结合完整架构评估 |
| 快速严格同步（`fast` + `crit.yml`）  | 目标 < 30s        | 已确认事务 0          | 需结合完整架构评估 |
| 两地三中心 + 同步                   | 取决于网络与所选 RTO 预设 | 取决于同步/严格模式       | 需结合完整架构评估 |
{.full-width}
...
|:----------------|:------------------|:------------------------------|:---------------|
| PCI-DSS 核心交易    | 目标 RTO<45s，异步可能丢失 | `fast` + 严格同步目标 RTO<30s、RPO=0 | 需按业务 BIA 与演练确认 |
| SOX 财务系统        | ✅                 | ✅                             | ✅ 默认即满足        |
| HIPAA EHR       | ✅                 | ✅                             | ✅ 默认即满足        |
| Basel III 核心银行  | 需配置同步             | `fast` + 严格同步目标 RTO<30s、RPO=0 | 需按监管与演练确认      |
...
```yaml
pg_rto: norm                          # 当前四个合法预设之一，目标 RTO < 45s
pg_rpo: 1048576                       # 候选从库采样落后阈值 1MiB，并非实际丢失硬上限
...
bootstrap:
  dcs:
    ...
    # 目标 RTO < 45s；实际异步数据损失取决于写入速率、TTL 与采样时机
```
...
4. **PostgreSQL + Patroni 完全胜任**：
   - 同步复制模式可实现 **RPO=0**
   - `fast` 预设的自动 Failover 目标为 **RTO<30秒**
   - 远超大多数合规标准的最低要求
