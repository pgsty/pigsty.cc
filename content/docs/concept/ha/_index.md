...
Pigsty 的 PostgreSQL 集群带有开箱即用的高可用方案，由 [**Patroni**](/docs/patroni)、[**Etcd**](https://etcd.io/) 和 [**HAProxy**](http://www.haproxy.org/) 提供核心能力。
...
默认 `norm` 模式的目标 RTO 为 45 秒内；异步复制的 `pg_rpo=1MiB` 是 Patroni 的候选从库采样落后阈值，并非实际丢失量硬上限。使用 `crit.yml` 的严格同步模式可让已确认事务在故障切换时保持 RPO = 0。以上行为可通过参数按实际硬件与可靠性要求 [**配置**](/docs/concept/ha/rto)。
...
* 将 [**数据安全**](/docs/concept/sec/data) C/IA 中的可用性提高到一个新高度：RPO ≈ 0，RTO < 45s。
