...
因此对于常规场景，通常可以接受一定量的数据丢失，以换取更高的可用性与性能。
...
[`pg_rpo`](/docs/pgsql/param#pg_rpo) 会被写入 Patroni 的 `maximum_lag_on_failover`，默认值为 `1048576`（1MiB）。它是**候选从库参与竞选时允许的采样落后阈值**，不是实际数据丢失量的硬上限。
...
| **RPO**    | <span class="text-warning">**可能丢失；默认候选阈值 1MiB**</span> | <span class="text-primary">**正常 = 0 / 降级后可能丢失**</span> |       <span class="text-success">**= 0**</span>        |
...
在这种情况下，可以通过 [**`pg_rpo`**](/docs/pgsql/param#pg_rpo) 调整候选从库的采样落后阈值；
