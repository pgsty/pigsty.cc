...
title: 访问控制
weight: 232
description: Pigsty 提供了标准的安全实践：密码与证书认证，开箱即用的权限模型，SSL加密网络流量，加密远程冷备份等。
...
- 离线只读访问 (`dbrole_offline`): 受限的只读访问角色（只能访问 [offline](/docs/pgsql/config#离线从库) 实例，通常是个人用户，ETL工具账号）
...
- [`pg_dbsu`](/docs/pgsql/param#pg_dbsu)：操作系统 dbsu 名称，默认为 postgres，最好不要更改它
- [`pg_dbsu_password`](/docs/pgsql/param#pg_dbsu_password)：dbsu 密码，默认为空字符串意味着不设置 dbsu 密码，最好不要设置。
- [`pg_replication_username`](/docs/pgsql/param#pg_replication_username)：postgres 复制用户名，默认为 `replicator`
- [`pg_replication_password`](/docs/pgsql/param#pg_replication_password)：postgres 复制密码，默认为 `DBUser.Replicator`
- [`pg_admin_username`](/docs/pgsql/param#pg_admin_username)：postgres 管理员用户名，默认为 `dbuser_dba`
- [`pg_admin_password`](/docs/pgsql/param#pg_admin_password)：postgres 管理员密码的明文，默认为 `DBUser.DBA`
- [`pg_monitor_username`](/docs/pgsql/param#pg_monitor_username)：postgres 监控用户名，默认为 `dbuser_monitor`
- [`pg_monitor_password`](/docs/pgsql/param#pg_monitor_password)：postgres 监控密码，默认为 `DBUser.Monitor`
...
数据库中新建对象的默认权限由参数 [`pg_default_privileges`](/docs/pgsql/param#pg_default_privileges) 所控制：
...
SQL 语句 [`ALTER DEFAULT PRIVILEGES`](https://www.postgresql.org/docs/current/sql-alterdefaultprivileges.html) 允许您设置将来创建的对象的权限。 它不会影响已经存在对象的权限，也不会影响非管理员用户创建的对象。
...
这些内容将会被 PG集群初始化模板 [`pg-init-template.sql`](https://github.com/pgsty/pigsty/blob/main/roles/pgsql/templates/pg-init-template.sql) 所使用，在集群初始化的过程中渲染并输出至 `/pg/tmp/pg-init-template.sql`。
...
1. [`{{ pg_dbsu }}`](/docs/pgsql/param#pg_dbsu)，默认为 `postgres`
2. [`{{ pg_admin_username }}`](/docs/pgsql/param#pg_admin_username)，默认为 `dbuser_dba`
...
- 如果 `owner` 参数存在，它作为数据库属主，替代默认的 [`{{ pg_dbsu }}`](/docs/pgsql/param#pg_dbsu)（通常也就是`postgres`）
