...
数据库主机全灭，备份依然健在；配合 AES-256 加密与 MinIO 的多节点纠删码，还能对抗仓库侧的磁盘故障与数据泄露。
...
| 高合规要求 | MinIO 版本控制 + 对象锁定 | 防篡改、防勒索：备份不可被覆盖或删除 |
{.full-width}
...
* **每日全量，保留两份**（本地仓库默认思路）：约 200 GB 备份 + 两天 WAL 归档 ≈ **2～3 倍** 数据库大小，
  换来一至两天的恢复窗口。
* **每周全量 + 每日增量，保留十四天**（MinIO 仓库默认思路）：约两份全量 + 十二份增量 + 两周 WAL ≈ **4～5 倍**
  数据库大小，换来一至两周的恢复窗口。
...
要到月底对账才暴露的错误，则需要以周计的窗口。默认的"本地一两天、远程两周"正是对这两类需求的回应。
...
对写入繁忙的库，"每周全量"意味着最坏情况下要重放整整一周的 WAL —— 这正是增量备份的价值：
...
**默认策略：本地仓库 + 每日全量**。零配置，简单可靠，恢复走本地磁盘速度最快，适合开发测试与容灾要求有限的场景：
...
**生产策略：MinIO 仓库 + 周全量日增量**。独立故障域，AES-256 加密，两周窗口，适合严肃生产环境：
...
这既是对备份完整性的端到端验证，也是对 RTO 的实测校准，而且对生产集群零风险。
