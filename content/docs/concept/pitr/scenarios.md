...
* **克隆恢复是默认选项**。它对生产零风险、可以反复尝试不同时间点、可以先验证再动手。
...
./pgsql-pitr.yml -l pg-test -e '{"pg_pitr": { "cluster": "pg-meta", "time": "2026-07-11 10:14:00+08" }}'
...
不影响生产、不修改历史、可审计可复现。配合 `immediate` 目标或 `action: pause`，
集群可以停在恢复点上供检查，而不推进到新时间线。
...
而它成立的前提只有一个：**备份仓库在灾难的故障域之外**。使用 [**远程备份仓库**](/docs/concept/pitr/tradeoff/) 时，
...
```bash
./pgsql-pitr.yml -l pg-meta    # 不指定目标即为 default：重放全部 WAL，恢复到最后一刻
```

值得指出的是：配置清单 + 远程备份仓库，就是重建整套环境所需的 **全部** ——
这也是把配置清单纳入版本控制、把备份放进独立故障域这两件事的终极意义。
...
克隆恢复给了您一个零风险的演练场：随时可以把任意集群的备份恢复成一套新集群，验证、计时、销毁。
