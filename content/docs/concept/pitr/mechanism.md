...
再持续保留此后产生的所有 WAL，就可以随时把数据库 **重放** 到这段历史所覆盖的任意时刻。
...
archive_timeout: 300                                          # 空闲时最多五分钟强制切段归档
```

两个细节值得注意：

* **`archive_timeout: 300`** 给恢复窗口的右边界上了一道保险：即使数据库几乎空闲、WAL 段迟迟写不满，
  每五分钟也会强制切换并归档一次。因此 PITR 的 RPO 上界约为"最近五分钟或最后几十 MB 的写入"。
* **异步归档**（`archive-async=y`）：pgBackRest 使用本地假脱机目录（`/pg/spool`）异步批量推送 WAL，
  避免归档吞吐成为主库写入的瓶颈 —— 归档的可靠性与主库的性能在这里解耦。
...
在 Pigsty 的默认策略下，本地仓库保留最近两个全量备份（每日全备时窗口约一至两天），
MinIO / S3 仓库按时间保留十四天。窗口的长短本质上是空间与需求的权衡，详见 [**策略权衡**](/docs/concept/pitr/tradeoff/)。
