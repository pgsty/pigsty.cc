...
- 如果指定了 **`-g`** 参数，将配置向导识别的默认密码替换为随机生成的强密码；仍需按 [**默认凭证清单**](/docs/concept/sec/compliance#默认凭证清单) 检查未覆盖的凭据。（**强烈推荐**）
...
3. **密码安全**：生产环境**务必**修改配置文件中的默认密码。可以使用 `-g` 参数随机化向导识别的凭据，并按 [**默认凭证清单**](/docs/concept/sec/compliance#默认凭证清单) 检查其余值。
