...
Pigsty 拥有一套开箱即用的权限模型，该模型与 [默认角色](#默认角色) 一起配合工作。
...
- 离线用户（`dbrole_offline`）类似只读用户，但访问受到限制，只允许访问 [离线实例](/docs/pgsql/config/cluster#离线从库)（`pg_role = 'offline'` 或 `pg_offline_query = true`）
- 由管理员用户创建的对象将具有正确的权限。
...
在 Pigsty 中，数据库（Database）层面的权限在 [数据库定义](/docs/pgsql/config/db#定义数据库) 中被涵盖。
