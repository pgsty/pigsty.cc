...
Demo 展示，开发测试，承载临时需求，进行无关紧要的计算分析任务时，使用单一数据库实例可能并没有太大问题。但这样的单机集群没有 [高可用](/docs/concept/ha)，当出现硬件故障时，您需要使用 [PITR](/docs/concept/pitr) 或其他恢复手段来确保集群的 RTO / RPO。为此，您可以考虑为集群添加若干个 [只读从库](#只读从库)
