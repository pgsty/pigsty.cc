...
title: HBA 规则
weight: 60
description: Pigsty 中 PostgreSQL 与 PgBouncer 的 HBA（Host-Based Authentication）规则配置参考。
...
HBA（Host-Based Authentication）控制“谁可以从哪里、以什么方式连接到数据库”。认证模型和默认规则说明见 [**身份认证**](/docs/concept/sec/auth)。
...
5. **TLS 前提**：`ssl`、`cert` 要求服务端 TLS；客户端仍需使用 `verify-full` 验证服务端身份
...
- [**HBA 管理**](/docs/pgsql/admin/hba/)：刷新、验证与故障排查
- [**用户配置**](/docs/pgsql/config/user/)：用户与角色配置
- [**访问控制**](/docs/pgsql/config/acl/)：角色体系与权限模型
- [**身份认证**](/docs/concept/sec/auth)：HBA、SCRAM、客户端证书与默认规则
- [**加密通信**](/docs/concept/sec/ca)：TLS 与服务端证书验证
