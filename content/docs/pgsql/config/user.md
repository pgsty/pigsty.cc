...
Pigsty 提供一套内置的访问控制 / [**ACL**](/docs/concept/sec/ac#角色体系) 模型，可以将以下默认业务角色分配给用户：
...
`dbrole_offline` 本身不会把用户限制到离线实例。若需要该边界，应为对应 HBA 规则设置 `role: offline`；详见 [**离线角色与实例隔离**](/docs/concept/sec/ac#离线角色与实例隔离)。
...
关于用户的访问权限，请参考 [**访问控制：角色体系**](/docs/concept/sec/ac#角色体系)。
