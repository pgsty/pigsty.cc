...
title: 时间点恢复
weight: 1105
description: 介绍 Pigsty 中 PostgreSQL 时间点恢复的实现架构，原理，利弊权衡与实现细节。
...
Pigsty 的 PostgreSQL 集群带有自动配置的时间点恢复（PITR）方案，基于备份组件 [**pgBackRest**](https://pgbackrest.org/) 与可选的对象存储仓库 [**MinIO**](https://min.io/) 提供。
...
除了 [**PITR**](/docs/pgsql/arch/pitr) 之外，您还可以在 Pigsty 中使用 [**延迟集群**](/docs/pgsql/config#延迟集群) 来解决人为失误或软件缺陷导致的数据误删误改问题。
...
具体操作，请参考 [**PGSQL管理：备份恢复**](/docs/pgsql/arch/pitr)。
...
默认情况下，Pigsty提供了两种预置[备份策略](/docs/pgsql/arch/pitr#备份策略)：默认使用本地文件系统备份仓库，在这种情况下每天进行一次全量备份，确保用户任何时候都能回滚至一天内的任意时间点。备选策略使用专用的 MinIO 集群或S3存储备份，每周一全备，每天一增备，默认保留两周的备份与WAL归档。
