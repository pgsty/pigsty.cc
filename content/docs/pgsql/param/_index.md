...
[`PG_BOOTSTRAP`](#pg_bootstrap) 参数组用于配置 PostgreSQL 集群初始化引导，包括 Patroni 高可用、存储路径、连接、编码等核心设置。
...
[默认](/docs/concept/sec/ac#角色体系) 的数据库用户及其凭据，生产环境必须修改这些用户的密码。
...
别名的具体定义可以在 [`roles/node_id/vars/`](https://github.com/pgsty/pigsty/tree/main/roles/node_id/vars) 中的 `pg_package_map` 查到，Pigsty 会先根据操作系统和架构解析别名，再将 `$v`/`${pg_version}` 替换为实际主版本 [`pg_version`](#pg_version)，最后安装真实的软件包。这样可以屏蔽不同发行版之间的包名差异。
...
以下为 PGSQL 引导阶段的可配置参数。内部变量 [`pg_data`](#pg_data) 固定表示 `/pg/data` 软链，不应在配置清单中覆盖；需要调整实际主数据目录位置时，请配置 [`pg_fs_main`](#pg_fs_main)。
...
该软链指向底层实际数据目录，在 Patroni 模板、维护脚本与清理流程中被多处引用，请不要在 `pigsty.yml` 中覆盖或修改它。需要调整实际主数据目录位置时，请配置 [`pg_fs_main`](#pg_fs_main)。参阅 [PGSQL文件结构](/docs/ref/fhs) 获取详细信息。
...
* [默认角色](/docs/concept/sec/ac#角色体系)
* [默认用户](/docs/concept/sec/ac#角色体系)
...
Pigsty 有一个内置的角色系统，请查看 [PGSQL 访问控制：角色体系](/docs/concept/sec/ac#角色体系) 了解详情。
...
Pigsty 基于默认角色系统提供相应的默认权限设置，请查看 [PGSQL 访问控制：默认权限](/docs/concept/sec/ac#默认权限) 了解详情。
...
默认规则面向受信内网中的常规部署，并非公网或合规场景的加固基线。默认的 `+dbrole_offline` 规则没有 `role` 字段，因此会应用到所有实例；若需实例隔离，应复制完整默认列表，并将该规则设为 `role: offline`。详见 [**身份认证**](/docs/concept/sec/auth) 与 [**访问控制**](/docs/concept/sec/ac#离线角色与实例隔离)。
