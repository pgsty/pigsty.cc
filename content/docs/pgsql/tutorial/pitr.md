...
如果使用 `pgsql-pitr.yml`，Pigsty v4.3 默认会保留归档设置。手工 `pgbackrest restore` 或使用旧脚本时，恢复结果可能会在 `postgresql.auto.conf` 中写入 `archive_mode = off`；如果您希望新领导者的写入归档到备份仓库，需要检查并重置该配置。
