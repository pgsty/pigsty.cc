...
- 请修改 MinIO 普通用户的密码：[**`minio_users`**`.`[pgbacrest]`.`secret_key`](/docs/minio/param#minio_users)
...
- 在 [**`pg_libs`**](/docs/pgsql/param#pg_libs) 中添加 `$lib/passwordcheck` 来强制密码策略。
...
- 有一个增强安全性的配置模板：[`security.yml`](https://github.com/pgsty/pigsty/blob/main/conf/demo/security.yml)
...
**启用数据校验和，以检测静默数据损坏。**
- [**`pg_checksum`**](/docs/pgsql/param#pg_checksum) 默认为 `off`，但建议开启。
