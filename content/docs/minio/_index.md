...
| 模式 | 说明 | 适用场景 |
|:-----|:-----|:---------|
| [**单机单盘**](config#单机单盘) (SNSD) | 单节点，单个数据目录 | 开发、测试、演示 |
| [**单机多盘**](config#单机多盘) (SNMD) | 单节点，多块磁盘 | 资源受限的小规模部署 |
| [**多机多盘**](config#多机多盘) (MNMD) | 多节点，每节点多块磁盘 | **生产环境推荐** |

此外，您还可以使用[多池部署](config#多池部署)来扩容现有集群，或部署[多套集群](config#多套集群)。
