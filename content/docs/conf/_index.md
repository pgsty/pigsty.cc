...
当前 `main` 分支（v4.5.0 开发版）在 `conf/` 目录提供 52 个独立的 YAML 配置模板，适用于不同的部署、测试与构建场景。最新正式发布版 v4.4.0 提供 48 个；新增的是 `demo/kafka`、`demo/mysql`、`demo/rustfs` 与 `ha/octo`。`app/supa.yml` 是指向 `supabase.yml` 的兼容软链接，不重复计数。
