...
{{< cards >}}
{{< card title="关于" link="/docs/about/" icon="fa-solid fa-circle-info" >}}
了解 Pigsty 项目本身的方方面面：功能特性、历史沿革、开源协议、隐私政策与社区动态。
...
{{< /card >}}
{{< card title="概念" link="/docs/concept/" icon="fa-solid fa-compass" >}}
...
{{< /card >}}
{{< card title="上手" link="/docs/setup/" icon="fa-solid fa-rocket" >}}
在你的笔记本或云服务器上拉起 Pigsty 单机版本，访问数据库服务与 Web 管理界面。
...
```bash {title="Terminal" copy="all" label="Pigsty 快速安装"}
...
{{< cards >}}
{{< card title="PGSQL" link="/docs/pgsql/" icon="fa-brands fa-postgresql" badge="核心" >}}
自治自愈的高可用 PostgreSQL 集群：HA、PITR、IaC、ACL 与监控齐备，海量扩展开箱即用。
...
{{< /card >}}
{{< card title="INFRA" link="/docs/infra/" icon="fa-solid fa-bank" badge="核心" >}}
Nginx、本地软件仓库、DNS、NTP，以及 VictoriaMetrics 与 Grafana 可观测性技术栈。

[配置](/docs/infra/config) [管理](/docs/infra/admin/) [剧本](/docs/infra/playbook) [监控](/docs/infra/monitor) [参数](/docs/infra/param)
{{< /card >}}
{{< card title="NODE" link="/docs/node/" icon="fa-solid fa-server" badge="核心" >}}
将主机节点纳管调整至期望状态：主机监控、日志收集、VIP 与 HAProxy 负载均衡。

[配置](/docs/node/config) [管理](/docs/node/admin) [剧本](/docs/node/playbook) [监控](/docs/node/monitor) [参数](/docs/node/param)
{{< /card >}}
{{< card title="ETCD" link="/docs/etcd/" icon="fa-solid fa-gears" badge="核心" >}}
可靠的分布式共识存储（DCS），为 PostgreSQL 高可用提供集群元数据支撑。
...
{{< cards >}}
{{< card title="MINIO" link="/docs/minio/" icon="fa-solid fa-boxes-stacked" badge="可选" >}}
S3 兼容的对象存储，可用作数据库备份的集中存储仓库。
{{< /card >}}
{{< card title="REDIS" link="/docs/redis/" icon="fa-solid fa-layer-group" badge="可选" >}}
高性能内存数据结构服务器，主从、集群、哨兵三种模式。
{{< /card >}}
{{< card title="DOCKER" link="/docs/docker/" icon="fa-brands fa-docker" badge="可选" >}}
容器运行时，一键拉起容器化的无状态软件与应用模板。
{{< /card >}}
{{< card title="JUICE" link="/docs/juice/" icon="fa-solid fa-folder-tree" badge="可选" >}}
JuiceFS 分布式文件系统，将 PostgreSQL 挂载为文件系统
{{< /card >}}
{{< card title="VIBE" link="/docs/vibe/" icon="fa-solid fa-laptop-code" badge="可选" >}}
AI 编程沙箱，Claude/Codex 开发环境，Agent 运行时
{{< /card >}}
{{< card title="KAFKA" link="/docs/kafka/" icon="fa-solid fa-share-nodes" badge="可选" >}}
Apache Kafka 4.x dynamic KRaft 消息队列集群，安全与监控齐备。
{{< /card >}}
{{< card title="MYSQL" link="/docs/mysql/" icon="fa-solid fa-fish" badge="可选" >}}
MySQL 数据库，广泛使用的关系型数据库管理系统。
{{< /card >}}
{{< card title="PILOT" link="/docs/pilot/" icon="fa-solid fa-flask-vial" badge="试点" >}}
试点功能模组：Kubernetes、DuckDB、MySQL、TigerBeetle……供尝鲜探索。
{{< /card >}}
{{< /cards >}}
...
{{< cards >}}
{{< card title="参考手册" link="/docs/ref/" icon="fa-solid fa-map" >}}
详尽的参考信息列表：操作系统、文件结构、参数、监控指标与同类产品对比。
...
{{< /card >}}
{{< card title="已打包扩展" link="/ext/" icon="fa-solid fa-puzzle-piece" >}}
{pgext_count} 个已打包 PostgreSQL 生态扩展：元数据、文档、下载与支持矩阵。

[扩展列表](/ext/list) [系统支持](/ext/os) [软件仓库](/docs/repo/)
{{< /card >}}
{{< card title="开箱模板" link="/docs/conf/" icon="fa-solid fa-cubes" >}}
开箱即用的集群配置模板，以及容器化的软件与应用模板。

[配置模板](/docs/conf/) [应用模板](/docs/app/)
{{< /card >}}
{{< card title="组件手册" link="/docs/pig/" icon="fa-solid fa-toolbox" >}}
Pigsty 所用周边组件与工具的中文文档：包管理、高可用、连接池、备份与监控。
