...
[`kafka_role`](/docs/kafka/param#kafka_role) 只接受三个值：
...
| 参数 | 默认值 | 用途 |
|:---|:---|:---|
| [`kafka_port`](/docs/kafka/param#kafka_port) | `9092` | Broker、客户端与 Broker 间通信 |
| [`kafka_controller_port`](/docs/kafka/param#kafka_controller_port) | `9093` | KRaft Controller 仲裁 |
| [`kafka_exporter_port`](/docs/kafka/param#kafka_exporter_port) | `9308` | 协议 Exporter HTTP 指标 |
| [`kafka_jmx_exporter_port`](/docs/kafka/param#kafka_jmx_exporter_port) | `9404` | JMX Exporter HTTP 指标 |
{.full-width}
...
跨故障域部署可以在所有 Broker-capable 节点上一致声明 [`kafka_rack`](/docs/kafka/param#kafka_rack)：
...
[`kafka_parameters`](/docs/kafka/param#kafka_parameters) 是唯一的 Broker 参数逃生舱，默认 `{}`，只渲染到 Broker-capable 节点。它适合 `num.partitions`、线程数、Buffer、保留与 Segment 等非角色自有键。
...
安全模式在 Bootstrap 后不能通过普通剧本切换。内部凭据与证书可以使用 [受保护轮换](/docs/kafka/playbook#受保护轮换)，但 `plaintext` 到 `scram` 的在线迁移仍需未来的显式状态机。
