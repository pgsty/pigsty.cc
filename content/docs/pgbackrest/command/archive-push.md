...
接收来自 PostgreSQL 的 WAL 段，并将其归档到各个仓库中（仓库路径由带索引的 `repo-path` 选项定义，详见 [**仓库选项**](/docs/pgbackrest/configuration/#repository-options) 章节）。WAL 段可以立即推送到归档，也可以根据 `archive-async` 的值先暂存在本地。配置了多个仓库时，`archive-push` 会尽量向所有仓库推送。

`archive-push` 由 PostgreSQL 负责配置和调用。相关示例请参见 [**配置归档**](/docs/pgbackrest/user-guide/#configure-archiving)。
...
- pgBackRest 会告知 PostgreSQL WAL 已成功归档，然后**将其丢弃**。
...
### Azure 仓库 URI 样式选项（`--repo-azure-uri-style`）
...
### S3 仓库 URI 样式选项（`--repo-s3-uri-style`）
...
将 NFS 挂载用作 `posix` 仓库时，适用的规则与 PostgreSQL 文档中描述的规则相同，详见 [**Creating a Database Cluster - File Systems**](https://www.postgresql.org/docs/current/creating-cluster.html#CREATING-CLUSTER-FILESYSTEM)。
