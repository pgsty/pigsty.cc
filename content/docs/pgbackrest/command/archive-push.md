...
接收来自 PostgreSQL 的 WAL 段，并将其归档到各个仓库中（仓库路径由带索引的 `repo-path` 选项定义，详见 [**仓库选项**](/docs/pgbackrest/configuration/#仓库选项) 章节）。WAL 段可以立即推送到归档，也可以根据 `archive-async` 的值先暂存在本地。配置了多个仓库时，`archive-push` 会尽量向所有仓库推送。

`archive-push` 由 PostgreSQL 负责配置和调用。相关示例请参见 [**配置归档**](/docs/pgbackrest/user-guide/#配置归档)。
